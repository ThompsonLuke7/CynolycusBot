...
from events.forward_guidance.config import EVENTS_PATH, ensure_data_dirs
from events.forward_guidance.data.ingest_events import ingest_event, load_events, load_events_from_csv, write_events
from events.forward_guidance.data.market_data import fetch_event_market_window
from events.forward_guidance.data.schema import EarningsEvent, event_from_record
...
def stage_discover_events(args: argparse.Namespace) -> None:
    from events.forward_guidance.config import DISCOVERED_EVENTS_CSV
    from events.forward_guidance.data.discover_events import discover_events, write_discovered_events
...
def stage_features(args: argparse.Namespace) -> None:
    events = _limit(_events_from_args(args), args.limit)
    from events.forward_guidance.features.build_matrix import build_feature_matrix
...
def stage_train(args: argparse.Namespace) -> None:
    from events.forward_guidance.models.train import train_models
...
def stage_inference(args: argparse.Namespace) -> None:
    events = _limit(_events_from_args(args), args.limit)
    from events.forward_guidance.inference.daily import score_events
...
def stage_backtest(args: argparse.Namespace) -> None:
    from events.forward_guidance.backtests.simulate import run_backtest
