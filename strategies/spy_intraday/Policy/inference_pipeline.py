...
from strategies.spy_intraday.Features.feature_matrix import build_feature_matrices, clean_feature_matrix
from strategies.spy_intraday.Features.feature_matrix_regime import AgentFeatureConfig, build_agent_feature_matrix
from strategies.spy_intraday.Features.feature_scaling import save_normalization_stats
...
def _run_ga_xgb(
    *,
    ...
    cmd = [sys.executable, str(REPO_ROOT / "strategies" / "spy_intraday" / "Models" / "ga_xgboost" / "train.py")]
...
def _run_agent_eval(
    *,
    ...
    cmd = [
        sys.executable,
        str(REPO_ROOT / "strategies" / "spy_intraday" / "Policy" / "Agent" / "run_eval.py"),
        "--data-csv",
        str(agent_csv),
        "--model-path",
        str(model_path),
...
def _run_meta_replay_eval(
    *,
    ...
    cmd = [
        sys.executable,
        "-m",
        "core.API.Alpaca_API.runners.replay_runner",
        "--data-path",
