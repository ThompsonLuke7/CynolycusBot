...
PER_RECORD_PATH = Path("meta_context/data/processed/news_catalyst_per_record.parquet")
OOF_BINARY_PATH = Path("meta_context/data/processed/catalyst_oof_target_expansion_10pct.parquet")
OOF_TRAJECTORY_PATH = Path("meta_context/data/processed/catalyst_oof_target_trajectory_code.parquet")
TRAINING_MATRIX_PATH = Path("meta_context/data/processed/catalyst_training_matrix.parquet")
...
def build(
    ...
    *,
    ...
    prod_scores = np.concatenate(scores) if scores else np.array([])
    if has_traj and traj_chunks:
        prod_traj = pd.concat(traj_chunks, ignore_index=True)
    else:
        prod_traj = pd.DataFrame(0.0, index=range(len(prod_df)), columns=list(TRAJECTORY_LABELS))
    ...
    df.loc[prod_idx, "catalyst_score"] = prod_scores
    if has_traj:
        for col in TRAJECTORY_LABELS:
            df.loc[prod_idx, f"p_{col}"] = prod_traj[col].values
    ...
    per_record_cols = ["record_id", "ticker", "timestamp", "source", "catalyst_score"]
    ...
    # Derive per-record flags used by the multiplicity / confluence aggregates
    ...
    agg_kwargs = dict(
        ...
        # Multiplicity / confluence features so the meta-ranker can see "3
        # independent bullish news on the same day" vs "1 noisy headline"
    ...
    # Source diversity AMONG high-score records — # of distinct sources where
    # catalyst_score >= HIGH_SCORE_THRESHOLD. Computed separately because the
    # groupby agg API doesn't easily support conditional nunique.
...
def reaggregate_from_cache(
    ...
    df_sorted = df.sort_values("catalyst_score", ascending=False)
    # Need catalyst_family/subtype for "top_family/top_subtype" — pull from news_records
