...
from signals.events.forward_guidance.data.ingest_events import load_events_from_csv
from signals.events.forward_guidance.data.schema import EarningsEvent
from signals.events.forward_guidance.features.build_matrix import build_feature_row
...
def score_feature_frame(features: pd.DataFrame, *, model_path: Path | str | None = None, meta_path: Path | str = MODEL_META_PATH) -> pd.DataFrame:
    ...
    if not cols:
        from signals.events.forward_guidance.features.build_matrix import feature_columns
