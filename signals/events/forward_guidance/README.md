...
```bash
python -m signals.events.forward_guidance.main --stage discover-events --start 2025-01-01 --end 2026-02-01 --discovery-source sec --limit 10
python -m signals.events.forward_guidance.main --stage ingest --events-csv events.csv --limit 3
python -m signals.events.forward_guidance.main --stage fetch-market --events-csv events.csv --limit 3
python -m signals.events.forward_guidance.main --stage features --events-csv events.csv
python -m signals.events.forward_guidance.models.train --model-kind xgboost
python -m signals.events.forward_guidance.inference.daily --events-csv today_events.csv
...
```bash
python -m signals.events.forward_guidance.main --stage features --events-csv events.csv --embeddings --finbert
```
