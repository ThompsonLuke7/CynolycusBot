...
Typical use from ``momentum_expansion/live``:
    ...
    from signals.news.live_scoring import score_headline
...
from signals.news.pipeline import parse_embedding
from signals.news.relations import classify_news_relations
from signals.news.schema import NewsRecord
...
def _finbert_query(text: str) -> dict[str, float] | None:
    try:
        from signals.news.nlp import finbert_scores
