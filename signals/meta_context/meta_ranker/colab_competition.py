...
class CompetitionConfig:
    ...
    # --- magnitude-aware training (opt-in; defaults preserve prior behavior) ---
    # Binary classifiers: scale_pos_weight for class imbalance. "auto" => neg/pos on train.
    scale_pos_weight: float | str | None = None
    # Per-row magnitude (e.g. mfe_up_atr). Positives are up-weighted (classifier
    # sample_weight) and graded (ranker relevance) by it; None disables both.
    magnitude_column: str | None = None
    magnitude_gain: float = 1.0          # >0 sharpens the magnitude weighting; <=0 disables
    magnitude_weight_cap: float = 3.0    # max per-row magnitude multiplier (classifier)
...
def magnitude_multiplier(frame: pd.DataFrame, cfg: CompetitionConfig) -> np.ndarray:
    """Per-row weight multiplier that emphasizes large-magnitude positives.

    Positive rows (target == positive_label) are scaled by their `magnitude_column`
    value relative to the positive median, capped at `magnitude_weight_cap`; rows at or
    below the median (and all negatives) stay at 1.0 (never down-weighted). Returns
    all-ones when magnitude weighting is not configured, so existing callers are unaffected.
    """
    if not cfg.magnitude_column or cfg.magnitude_column not in frame.columns or cfg.magnitude_gain <= 0:
        return np.ones(len(frame), dtype=float)
    y = frame[cfg.target_column].to_numpy()
    positive = resolve_positive_label(y, cfg.positive_label)
    pos_mask = y == positive
    mult = np.ones(len(frame), dtype=float)
    if not pos_mask.any():
        return mult
    mag = pd.to_numeric(frame[cfg.magnitude_column], errors="coerce").to_numpy(float)
    ref = np.nanmedian(mag[pos_mask])
    if not np.isfinite(ref) or ref <= 0:
        return mult
    scaled = 1.0 + cfg.magnitude_gain * (mag[pos_mask] / ref - 1.0)
    mult[pos_mask] = np.clip(np.nan_to_num(scaled, nan=1.0), 1.0, cfg.magnitude_weight_cap)
    return mult
...
def sample_weights(frame: pd.DataFrame, cfg: CompetitionConfig) -> np.ndarray | None:
    ...
    w = w * magnitude_multiplier(frame, cfg)
    return w.astype(np.float32)


def resolve_scale_pos_weight(train_df: pd.DataFrame, cfg: CompetitionConfig, n_classes: int) -> float | None:
    """scale_pos_weight for binary classifiers: numeric passthrough or "auto" => neg/pos."""
    if cfg.scale_pos_weight is None or n_classes != 2:
        return None
    if isinstance(cfg.scale_pos_weight, (int, float)):
        return float(cfg.scale_pos_weight)
    y = train_df[cfg.target_column].to_numpy()
    positive = resolve_positive_label(y, cfg.positive_label)
    pos = float((y == positive).sum())
    neg = float(len(y) - pos)
...
def train_relevance(frame: pd.DataFrame, cfg: CompetitionConfig) -> np.ndarray:
    """Ranker *training* target. Graded by magnitude when enabled, else binary relevance().

    Positives are split into integer grades 1..graded_relevance_bins by within-positive
    magnitude quantile, so NDCG/lambdarank rank large-magnitude pivots above small ones.
    Metrics still use the binary relevance() so NDCG/precision@k stay comparable across runs.
    """
    base = relevance(frame, cfg)
    if not (cfg.graded_relevance and cfg.magnitude_column and cfg.magnitude_column in frame.columns):
        return base
    pos_mask = base > 0
    grades = np.zeros(len(frame), dtype=np.float32)
    if pos_mask.any():
        bins = max(1, int(cfg.graded_relevance_bins))
        mag = pd.to_numeric(frame[cfg.magnitude_column], errors="coerce")
        pct = mag[pos_mask].rank(pct=True).to_numpy()
        g = np.ceil(np.nan_to_num(pct, nan=1.0 / bins) * bins)
        grades[np.flatnonzero(pos_mask)] = np.clip(g, 1, bins)
    return grades
...
def fit_family(
    ...
    if family == "xgb_classifier":
        ...
        spw = resolve_scale_pos_weight(train_df, cfg, n_classes)
        if spw is not None:
    ...
    if family == "xgb_ranker":
        ...
        model.fit(
            rank_train[feats].to_numpy(np.float32), train_relevance(rank_train, cfg), group=train_group,
            eval_set=[(rank_val[feats].to_numpy(np.float32), train_relevance(rank_val, cfg))], eval_group=[val_group], verbose=50,
        )
        return model
    if family in {"lgbm_classifier", "lgbm_ranker", "lgbm_regressor"}:
        ...
        if family == "lgbm_classifier":
            params = lgbm_classifier_params(seed, n_classes, cfg)
            spw = resolve_scale_pos_weight(train_df, cfg, n_classes)
            if spw is not None:
                params["scale_pos_weight"] = spw
        ...
        model.fit(
            rank_train[feats], train_relevance(rank_train, cfg), group=train_group,
            eval_set=[(rank_val[feats], train_relevance(rank_val, cfg))], eval_group=[val_group],
            callbacks=[lgb.early_stopping(early_rounds, verbose=True)],
        )
        return model
