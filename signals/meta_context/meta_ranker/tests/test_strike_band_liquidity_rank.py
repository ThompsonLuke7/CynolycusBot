...
# A real Alpaca latestQuote carries the instant it was observed. select_option
# refuses an undated market outright rather than stamping it with `now`, so the
# fixture has to supply one or every strike in the band reads as unusable.
_QUOTE_AT = "2026-08-14T18:30:00Z"
    ...
    def get_option_snapshots(self, *_a, **_k):
        ...
        for strike, delta, _oi, _vol in self.band:
            ...
            if strike not in self.quoteless:
                snap["latestQuote"] = {"bp": 4.0, "ap": 4.2, "t": _QUOTE_AT}
