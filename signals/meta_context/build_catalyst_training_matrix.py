...
OUTPUT_PATH = Path("signals/meta_context/data/processed/catalyst_training_matrix.parquet")
MANIFEST_PATH = Path("signals/meta_context/data/processed/catalyst_training_manifest.json")
...
#   .venv/bin/python -c "import pandas as pd; m=pd.read_parquet('signals/meta_context/data/processed/catalyst_training_matrix.parquet'); m=m.sort_values('timestamp').reset_index(drop=True); print(m.iloc[70000]['timestamp'], m.iloc[90000]['timestamp'])"
...
def build() -> pd.DataFrame:
    ...
    nr_path = Path("signals/news/data/processed/news_records.parquet")
    if nr_path.exists():
