...
Examples:
    python -m forward_guidance.main --stage discover-events --start 2025-01-01 --end 2026-02-01
    python -m forward_guidance.main --stage ingest --events-csv events.csv
    python -m forward_guidance.main --stage fetch-market --events-csv events.csv --limit 5
    python -m forward_guidance.main --stage features --events-csv events.csv
    python -m forward_guidance.main --stage train --model-kind xgboost
    python -m forward_guidance.main --stage inference --events-csv today_events.csv
"""
...
from forward_guidance.config import EVENTS_PATH, ensure_data_dirs
from forward_guidance.data.ingest_events import ingest_event, load_events, load_events_from_csv, write_events
from forward_guidance.data.market_data import fetch_event_market_window
...
def stage_discover_events(args: argparse.Namespace) -> None:
    from forward_guidance.config import DISCOVERED_EVENTS_CSV
    from forward_guidance.data.discover_events import discover_events, write_discovered_events
...
def stage_features(args: argparse.Namespace) -> None:
    events = _limit(_events_from_args(args), args.limit)
    from forward_guidance.features.build_matrix import build_feature_matrix
...
def stage_train(args: argparse.Namespace) -> None:
    from forward_guidance.models.train import train_models
...
def stage_inference(args: argparse.Namespace) -> None:
    events = _limit(_events_from_args(args), args.limit)
    from forward_guidance.inference.daily import score_events
...
def stage_backtest(args: argparse.Namespace) -> None:
    from forward_guidance.backtests.simulate import run_backtest
