...
- Event caches live under `forward_guidance/data/raw/{ticker}/{earnings_date}/`.
...
```bash
python -m forward_guidance.main --stage discover-events --start 2025-01-01 --end 2026-02-01 --discovery-source sec --limit 10
python -m forward_guidance.main --stage ingest --events-csv events.csv --limit 3
python -m forward_guidance.main --stage fetch-market --events-csv events.csv --limit 3
python -m forward_guidance.main --stage features --events-csv events.csv
python -m forward_guidance.models.train --model-kind xgboost
python -m forward_guidance.inference.daily --events-csv today_events.csv
python -m UI.forward_guidance_dashboard
```

The `discover-events` stage writes:

- `forward_guidance/data/processed/discovered_earnings_events.csv`
- `forward_guidance/data/processed/earnings_events.parquet`
...
```bash
python -m forward_guidance.main --stage features --events-csv events.csv --embeddings --finbert
```
