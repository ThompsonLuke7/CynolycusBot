...
class SwingDashboardHandler(BaseHTTPRequestHandler):
    ...
    def _write_json(self, payload: dict, status: int = HTTPStatus.OK) -> None:
        ...
        try:
            ...
            # Client hung up before we finished responding — e.g. a control POST
            # whose caller timed out during the multi-second session warmup.
            # Nothing to send; drop the connection quietly instead of letting an
            # unhandled BrokenPipe spam a traceback from the request thread.
