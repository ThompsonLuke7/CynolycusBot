...
from social_attention.config import AMBIGUOUS_BARE_TICKERS, REDDIT_MENTIONS_PATH, REDDIT_POSTS_PATH, REPO_ROOT
...
def load_universe_tickers() -> set[str]:
    tickers: set[str] = set()
    candidates = [
        REPO_ROOT / "meta_context" / "data" / "processed" / "context_backtest_universe.csv",
        REPO_ROOT / "multi_ticker_swing" / "config" / "swing_trader_universe_v3.csv",
        REPO_ROOT / "multi_ticker_swing" / "config" / "universe.csv",
    ]
