...
def main() -> None:
    ...
    df["theme_score"] = score
    tradable = df["is_tradable"].fillna(False).astype(bool)
    df["theme_rank"] = np.nan
    df.loc[tradable, "theme_rank"] = df.loc[tradable].groupby("date")["theme_score"].rank(ascending=False, method="first")
    df["watchlist_rank"] = np.nan
    df.loc[~tradable, "watchlist_rank"] = df.loc[~tradable].groupby("date")["theme_score"].rank(ascending=False, method="first")
