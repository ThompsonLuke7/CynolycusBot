...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Compare 1m entries using breakout-or-adverse-ATR fallback vs next-10m-open entries "
            "using the same hybrid exit rule."
        )
    )
    parser.add_argument(
        "--meta-matrix",
    ...
    parser.add_argument(
        ...
        default=1.0,
        help=(
            "Adverse ATR fallback for 1m entries after a 10m signal. "
            "Long fallback triggers at signal_close - multiple*ATR; short fallback at signal_close + multiple*ATR."
        ),
    )
    parser.add_argument(
        "--trace-out",
        default="Data/inference/spy/10min/meta/meta_trace_hybrid_breakout_or_adverse_atr_compare.csv",
        help="Shared 10m trace CSV with probabilities.",
    )
    parser.add_argument(
        "--actual-events-out",
        default="Data/inference/spy/10min/meta/meta_events_hybrid_exit_actual_1m_breakout_or_adverse_atr.csv",
        help="Actual 1m breakout-or-adverse-ATR events CSV.",
    )
    parser.add_argument(
        "--baseline-events-out",
    ...
    parser.add_argument(
        "--summary-out",
        default="Data/inference/spy/10min/meta/hybrid_exit_1m_breakout_or_adverse_atr_vs_next_open_summary.csv",
        help="Summary metrics CSV.",
    )
    parser.add_argument(
        "--equity-out",
        default="Data/inference/spy/10min/plots/hybrid_exit_1m_breakout_or_adverse_atr_vs_next_open_equity.png",
        help="Equity comparison PNG.",
    )
    parser.add_argument(
        "--out-dir",
        default="Data/inference/spy/10min/plots/hybrid_exit_1m_breakout_or_adverse_atr_vs_next_open_sessions",
...
def _run_hybrid(
    *,
    ...
    long_adverse_trigger = np.nan
    short_adverse_trigger = np.nan
    ...
    for idx, (_, row) in enumerate(meta_df.iterrows()):
        ...
        if not long_active:
            if long_valid_signal:
                ...
                signal_close = float(row.get("close", np.nan))
                signal_atr = float(row.get("atr", np.nan))
                long_adverse_trigger = (
                    signal_close - float(adverse_entry_atr) * signal_atr
                    if np.isfinite(signal_close) and np.isfinite(signal_atr) and signal_atr > 0.0
                    else np.nan
                )
        ...
        for _, bar in interval.iterrows():
            ...
            elif entry_mode == "breakout_or_adverse_atr":
                if (
                    long_intent_active
                    and (not long_active)
                    and (not entered_long_this_interval)
                    ...
                    and (
                        (
                            np.isfinite(long_ref_high)
                            and np.isfinite(bar_high)
                            and bar_high >= float(long_ref_high)
                        )
                        or (
                            np.isfinite(long_adverse_trigger)
                            and np.isfinite(bar_low)
                            and bar_low <= float(long_adverse_trigger)
                        )
                    )
                ):
                    breakout_hit = bool(
                    ...
                    if breakout_hit and adverse_hit:
                        breakout_dist = abs(float(long_ref_high) - float(bar_open)) if np.isfinite(bar_open) else 0.0
                        adverse_dist = abs(float(long_adverse_trigger) - float(bar_open)) if np.isfinite(bar_open) else 0.0
                        prefer_breakout = breakout_dist <= adverse_dist
                    else:
                        prefer_breakout = breakout_hit
                    trigger = float(long_ref_high) if prefer_breakout and np.isfinite(long_ref_high) else float(long_adverse_trigger)
                    fill_price = max(trigger, float(bar_open)) if np.isfinite(bar_open) else trigger
                    long_active = True
                    long_intent_active = False
                    entered_long_this_interval = True
                    long_agent._set_trade_entry(position=1, row=long_signal_row, entry_price=fill_price)
                    events.append(
                        {
                            ...
                            "event": "enter_long",
                            "price": fill_price,
                            "reason": "breakout" if prefer_breakout else "adverse_atr",
                        }
                    )
                if (
                    short_intent_active
                    ...
                    and (
                        (
                            np.isfinite(short_ref_low)
                            and np.isfinite(bar_low)
                            and bar_low <= float(short_ref_low)
                        )
                        or (
                            np.isfinite(short_adverse_trigger)
                            and np.isfinite(bar_high)
                            and bar_high >= float(short_adverse_trigger)
                        )
                    )
                ):
                    breakout_hit = bool(
                    ...
                    if breakout_hit and adverse_hit:
                        breakout_dist = abs(float(short_ref_low) - float(bar_open)) if np.isfinite(bar_open) else 0.0
                        adverse_dist = abs(float(short_adverse_trigger) - float(bar_open)) if np.isfinite(bar_open) else 0.0
                        prefer_breakout = breakout_dist <= adverse_dist
                    else:
                        prefer_breakout = breakout_hit
                    trigger = float(short_ref_low) if prefer_breakout and np.isfinite(short_ref_low) else float(short_adverse_trigger)
                    fill_price = min(trigger, float(bar_open)) if np.isfinite(bar_open) else trigger
                    short_active = True
                    short_intent_active = False
                    entered_short_this_interval = True
                    short_agent._set_trade_entry(position=-1, row=short_signal_row, entry_price=fill_price)
        ...
        trace_rows.append(
            {
                ...
                "long_adverse_trigger": float(long_adverse_trigger) if np.isfinite(long_adverse_trigger) else np.nan,
                "short_adverse_trigger": float(short_adverse_trigger) if np.isfinite(short_adverse_trigger) else np.nan,
...
def _save_equity_plot(*, actual_eq: pd.DataFrame, baseline_eq: pd.DataFrame, save_path: Path, symbol: str, adverse_entry_atr: float) -> None:
    fig, ax = plt.subplots(figsize=(16, 8))
    x_break = pd.to_datetime(actual_eq["timestamp"], utc=True).dt.tz_convert("America/New_York")
    x_base = pd.to_datetime(baseline_eq["timestamp"], utc=True).dt.tz_convert("America/New_York")
    ax.plot(x_break, actual_eq["buy_hold"], color="#444444", linewidth=1.5, label="buy_hold")
    ax.plot(
        x_break,
        actual_eq["net_1x_style"],
        color="#1565C0",
        linewidth=2.0,
        label=f"1m_breakout_or_adverse_{adverse_entry_atr:.2f}atr_net_1x",
    )
    ax.plot(x_base, baseline_eq["net_1x_style"], color="#2E7D32", linewidth=2.0, label="next_10m_open_net_1x")
    ax.plot(
        x_break,
        actual_eq["breakout_or_baseline"],
        color="#8E24AA",
        linewidth=1.4,
        label=f"1m_breakout_or_adverse_{adverse_entry_atr:.2f}atr_full_gross",
    )
    ax.plot(x_base, baseline_eq["breakout_or_baseline"], color="#C62828", linewidth=1.4, label="next_10m_open_full_gross")
    ax.set_title(f"{symbol} | hybrid exit | 1m breakout or adverse ATR vs next 10m open")
    ...
    *,
    ...
    for chunk_idx in range(chunks):
        ...
        for idx, session_date in enumerate(dates):
            ...
            ax_price.set_title(
                f"{symbol} | {session_date.date()} | hybrid exit | 1m breakout or adverse {adverse_entry_atr:.2f} ATR vs next 10m open"
            )
        ...
        fig.tight_layout()
        out_path = out_dir / f"{symbol.lower()}_hybrid_1m_breakout_or_adverse_atr_part{chunk_idx + 1}.png"
...
def main() -> None:
    ...
    trace_df, actual_events = _run_hybrid(
        ...
        entry_mode="breakout_or_adverse_atr",
        adverse_entry_atr=float(args.adverse_entry_atr),
    )
    ...
    summary = pd.DataFrame(
        [
            {
                "regime": f"1m_breakout_or_adverse_{float(args.adverse_entry_atr):.2f}atr_hybrid_exit",
                **_event_metrics(actual_events),
            },
