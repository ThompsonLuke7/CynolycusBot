...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare baseline exits vs baseline + profit-protect giveback exits."
    )
    parser.add_argument(
        "--meta-matrix",
    ...
    parser.add_argument("--profit-protect-arm-atr", type=float, default=2.0, help="Arm profit-protect after this much MFE in ATR.")
    parser.add_argument("--profit-protect-giveback-long", type=float, default=0.75, help="Exit long after this much giveback in ATR once armed.")
    parser.add_argument("--profit-protect-giveback-short", type=float, default=1.0, help="Exit short after this much giveback in ATR once armed.")
    parser.add_argument(
        "--trace-out",
        default="Data/inference/spy/10min/meta/meta_trace_baseline_profit_protect_compare.csv",
    ...
    parser.add_argument(
        "--variant-events-out",
        default="Data/inference/spy/10min/meta/meta_events_baseline_exit_profit_protect.csv",
        help="Variant event CSV.",
    )
    parser.add_argument(
        "--summary-out",
        default="Data/inference/spy/10min/meta/baseline_profit_protect_compare_summary.csv",
        help="Summary metrics CSV.",
    )
    parser.add_argument(
        "--equity-out",
        default="Data/inference/spy/10min/plots/baseline_profit_protect_compare_equity.png",
        help="Equity comparison PNG.",
    )
    parser.add_argument(
        "--out-dir",
        default="Data/inference/spy/10min/plots/baseline_profit_protect_compare_sessions",
...
def _run_regime(
    *,
    ...
    opposite_dominance_delta: float,
    profit_protect_arm_atr: float | None,
    profit_protect_giveback_long: float | None,
    profit_protect_giveback_short: float | None,
    ...
    for idx, (_, row) in enumerate(meta_df.iterrows()):
        ...
        for _, bar in interval.iterrows():
            ...
            if profit_protect_arm_atr is not None and float(profit_protect_arm_atr) > 0.0:
                if long_active and np.isfinite(long_entry_price) and np.isfinite(long_entry_atr) and long_entry_atr > 0.0:
                    ...
                    realized_run_atr = (
                        (float(bar_open) - long_entry_price) / long_entry_atr
                        if np.isfinite(bar_open)
                        else float("nan")
                    )
                    giveback_atr = (
                        long_mfe_atr - realized_run_atr
                        if np.isfinite(long_mfe_atr) and np.isfinite(realized_run_atr)
                        else float("nan")
                    )
                    if (
                        np.isfinite(long_mfe_atr)
                        and long_mfe_atr >= float(profit_protect_arm_atr)
                        and np.isfinite(giveback_atr)
                        and np.isfinite(profit_protect_giveback_long)
                        and giveback_atr >= float(profit_protect_giveback_long)
                    ):
                        fill_price = float(bar_open) if np.isfinite(bar_open) else float("nan")
                        if np.isfinite(fill_price):
                            ...
                            long_entry_atr = float("nan")
                            long_favorable_anchor = float("nan")
                            events.append({"timestamp": bar_ts, "symbol": symbol, "event": "exit_long", "price": fill_price, "reason": "profit_protect"})
                if short_active and np.isfinite(short_entry_price) and np.isfinite(short_entry_atr) and short_entry_atr > 0.0:
                    ...
                    realized_run_atr = (
                        (short_entry_price - float(bar_open)) / short_entry_atr
                        if np.isfinite(bar_open)
                        else float("nan")
                    )
                    giveback_atr = (
                        short_mfe_atr - realized_run_atr
                        if np.isfinite(short_mfe_atr) and np.isfinite(realized_run_atr)
                        else float("nan")
                    )
                    if (
                        np.isfinite(short_mfe_atr)
                        and short_mfe_atr >= float(profit_protect_arm_atr)
                        and np.isfinite(giveback_atr)
                        and np.isfinite(profit_protect_giveback_short)
                        and giveback_atr >= float(profit_protect_giveback_short)
                    ):
                        fill_price = float(bar_open) if np.isfinite(bar_open) else float("nan")
                        if np.isfinite(fill_price):
                            short_active = False
                            ...
                            short_entry_atr = float("nan")
                            short_favorable_anchor = float("nan")
                            events.append({"timestamp": bar_ts, "symbol": symbol, "event": "exit_short", "price": fill_price, "reason": "profit_protect"})
...
def _event_metrics(events: pd.DataFrame) -> dict[str, float]:
    ...
    pp_count = int(events["reason"].astype(str).eq("profit_protect").sum()) if "reason" in events.columns else 0
    return {
        ...
        "profit_protect_exit_count": float(pp_count),
    }
...
def _save_equity_plot(
    *,
    ...
    save_path: Path,
    symbol: str,
    profit_protect_arm_atr: float,
    profit_protect_giveback_long: float,
    profit_protect_giveback_short: float,
) -> None:
    ...
    ax.plot(
        ...
        linewidth=2.0,
        label=(
            f"baseline_plus_profit_protect_a{profit_protect_arm_atr:.2f}"
            f"_gl{profit_protect_giveback_long:.2f}_gs{profit_protect_giveback_short:.2f}_net_1x"
        ),
    )
    ...
    ax.plot(
        ...
        color="#8E24AA",
        linewidth=1.4,
        label="baseline_plus_profit_protect_full_gross",
    )
    ax.set_title(f"{symbol} | next 10m open entries | baseline vs profit-protect exit")
    ...
    *,
    ...
    for chunk_idx in range(chunks):
        ...
        for idx, session_date in enumerate(dates):
            ...
            ax_price.set_title(
                f"{symbol} | {session_date.date()} | baseline vs profit-protect "
                f"(arm {profit_protect_arm_atr:.2f}, long gb {profit_protect_giveback_long:.2f}, short gb {profit_protect_giveback_short:.2f})"
            )
        ...
        fig.tight_layout()
        out_path = out_dir / f"{symbol.lower()}_baseline_profit_protect_part{chunk_idx + 1}.png"
...
def main() -> None:
    ...
    _, variant_events = _run_regime(
        ...
        urgent_exit_delta=float(args.urgent_exit_delta),
        opposite_dominance_delta=float(args.opposite_dominance_delta),
        profit_protect_arm_atr=float(args.profit_protect_arm_atr),
        profit_protect_giveback_long=float(args.profit_protect_giveback_long),
        profit_protect_giveback_short=float(args.profit_protect_giveback_short),
    )
    ...
    summary = pd.DataFrame(
        [
            ...
            {
                "regime": (
                    f"baseline_plus_profit_protect_a{float(args.profit_protect_arm_atr):.2f}"
                    f"_gl{float(args.profit_protect_giveback_long):.2f}"
                    f"_gs{float(args.profit_protect_giveback_short):.2f}"
                ),
