...
DEFAULT_OUT = DEFAULT_ANALYSIS_DIR / "phase4_entry_overlay_policy_compare_summary.csv"
DEFAULT_EVENTS_OUT = DEFAULT_ANALYSIS_DIR / "phase4_entry_overlay_policy_compare_events.csv"
...
def _combine_prob(df: pd.DataFrame, side: str) -> pd.Series:
    ...
    for col in candidates:
        ...
        out = vals if out is None else out.combine_first(vals)
    if out is None:
...
def _load_signal_frame(path: Path) -> pd.DataFrame:
    ...
    df["atr_proxy"] = _daily_atr_proxy(df)
    keep = [
        "timestamp",
        "available_ts",
        "open",
...
def _load_one_min(path: Path, start: pd.Timestamp, end: pd.Timestamp) -> pd.DataFrame:
    ...
    df = df[
        (df["timestamp"] >= start - pd.Timedelta(days=2))
...
def _daily_atr_proxy(frame: pd.DataFrame) -> pd.Series:
    tr = (pd.to_numeric(frame["high"], errors="coerce") - pd.to_numeric(frame["low"], errors="coerce")).abs()
    return tr.rolling(14, min_periods=3).mean().bfill().ffill().fillna(0.5).clip(lower=0.05)
...
def _base_thresholds(policy: str) -> tuple[float, float]:
    if "l042" in policy:
        return 0.42, 0.65
    return 0.35, 0.65


def _continuation_source_threshold(policy: str, *, trend_side: str) -> float:
    if trend_side == "long":
        return 0.15
    long_thr, _short_thr = _base_thresholds(policy)
    return long_thr


def _thresholds_for_row(policy: str, row: pd.Series) -> tuple[float, float]:
    long_thr, short_thr = _base_thresholds(policy)
    if policy.startswith("test_b"):
        regime = str(row.get("trend_regime", "neutral"))
        if regime == "bullish":
            short_thr = max(short_thr, 0.65)
        elif regime == "bearish":
            long_thr = max(long_thr, 0.65)
        else:
            long_thr = max(long_thr, 0.50)
            short_thr = max(short_thr, 0.75)
    if policy.startswith("test_c"):
        regime = str(row.get("trend_regime", "neutral"))
        if regime == "bullish":
            short_thr = max(short_thr, 0.65)
        elif regime == "bearish":
...
def _countertrend_allowed(policy: str, *, side: str, row: pd.Series) -> bool:
    if not (policy.startswith("test_b") or policy.startswith("test_c")):
        return True
    regime = str(row.get("trend_regime", "neutral"))
    if regime == "bullish" and side == "short":
        return _as_float(row.get("p_enter_short")) >= 0.65
    if regime == "bearish" and side == "long":
        return _as_float(row.get("p_enter_long")) >= 0.65
    if regime == "neutral":
        long_thr, short_thr = _thresholds_for_row(policy, row)
        prob = _as_float(row.get(f"p_enter_{side}"))
        return prob >= (long_thr if side == "long" else short_thr)
    return True


def _continuation_candidate(policy: str, row: pd.Series, *, side: str, threshold: float) -> PendingSetup | None:
    if not policy.startswith("test_c"):
        return None
    regime = str(row.get("trend_regime", "neutral"))
    ...
    if not all(math.isfinite(x) for x in (close, high, low, ema_fast, ema_slow, atr)):
        return None

    setup_ts = row["timestamp"]
    expires_at = setup_ts + pd.Timedelta(minutes=10 * 5)
    if side == "long" and regime == "bullish":
        opp_prob = _as_float(row.get("p_enter_short"))
        shallow = low >= ema_fast - 0.75 * atr
        trend_ok = close >= ema_fast and ema_fast >= ema_slow
        if trend_ok and shallow and opp_prob >= threshold:
            return PendingSetup(
                side="long",
                ref=high,
                setup_ts=setup_ts,
                ...
                source_side="short_failed",
            )
    if side == "short" and regime == "bearish":
        opp_prob = _as_float(row.get("p_enter_long"))
        shallow = high <= ema_fast + 0.75 * atr
        trend_ok = close <= ema_fast and ema_fast <= ema_slow
        if trend_ok and shallow and opp_prob >= threshold:
            return PendingSetup(
                side="short",
...
def _update_setups(
    ...
    *,
    ...
    counts = {"swing_setups": 0, "countertrend_vetoes": 0, "continuation_setups": 0}
    ...
    for side, valid, prob, threshold, ref in (
        ...
        if refresh and math.isfinite(ref):
            ...
            counts["swing_setups"] += 1
        above[side] = bool(prob >= threshold)

    if policy.startswith("test_c"):
        cont_long = _continuation_candidate(
            policy,
            row,
            side="long",
            threshold=_continuation_source_threshold(policy, trend_side="long"),
        )
        if cont_long is not None:
            pending["long"] = cont_long
            counts["continuation_setups"] += 1
        cont_short = _continuation_candidate(
            policy,
            row,
            side="short",
            threshold=_continuation_source_threshold(policy, trend_side="short"),
        )
        if cont_short is not None:
            pending["short"] = cont_short
            counts["continuation_setups"] += 1
    return counts
...
def _run_policy(
    *,
    ...
    counters = {"swing_setups": 0, "countertrend_vetoes": 0, "continuation_setups": 0}
    last_day = None
...
def _big_trend_days(decisions: pd.DataFrame) -> set:
    daily = decisions.copy()
    daily["day"] = daily["timestamp"].dt.date
    rows = []
    for day, group in daily.groupby("day"):
        if group.empty:
            continue
        open_ = _as_float(group.iloc[0]["open"])
        close = _as_float(group.iloc[-1]["close"])
        atr = float(pd.to_numeric(group["atr_proxy"], errors="coerce").median())
        if math.isfinite(open_) and math.isfinite(close) and math.isfinite(atr) and atr > 0:
            rows.append((day, abs(close - open_) / atr))
    return {day for day, move_atr in rows if move_atr >= 2.0}
...
def _metrics(
    ...
    *,
    ...
    trend_days = _big_trend_days(decisions)
    traded_days = {pd.Timestamp(e["entry_ts"]).date() for e in events}
    countertrend_losers = sum(
        1
        for e in events
        if (
            (e["side"] == "long" and e["entry_regime"] == "bearish")
            or (e["side"] == "short" and e["entry_regime"] == "bullish")
        )
        and float(e["return"]) <= 0.0
    )
    continuation_events = [e for e in events if e.get("entry_kind") == "continuation"]
    swing_events = [e for e in events if e.get("entry_kind") == "swing"]
    out = {
        "policy": policy,
        "window": label,
        ...
        "swing_trades": int(len(swing_events)),
        "continuation_trades": int(len(continuation_events)),
        ...
        "countertrend_losers": int(countertrend_losers),
        "big_trend_days": int(len(trend_days)),
        "traded_big_trend_days": int(len(trend_days & traded_days)),
        "missed_big_trend_days": int(len(trend_days - traded_days)),
        **{key: int(val) for key, val in counters.items()},
    }
    if continuation_events:
        cont_returns = np.array([float(e["return"]) for e in continuation_events], dtype=float)
        out["continuation_avg_return"] = float(np.nanmean(cont_returns))
        out["continuation_win_rate"] = float(np.nanmean(cont_returns > 0))
    else:
        out["continuation_avg_return"] = float("nan")
        out["continuation_win_rate"] = float("nan")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Compare swing-only, countertrend-veto, and continuation overlay entries.")
    ...
    parser.add_argument(
        "--window-scope",
        choices=["recent", "full", "both"],
        default="both",
        help="Run recent window, full window, or both.",
    )
    parser.add_argument("--setup-max-bars", type=int, default=4)
    parser.add_argument("--cutoff-hhmm", default="13:00")
    parser.add_argument(
        "--policies",
        default="test_a_swing_only_l035_s065,test_a_swing_only_l042_s065,test_b_countertrend_veto_l035_s065,test_b_countertrend_veto_l042_s065,test_c_continuation_overlay_l035_s065,test_c_continuation_overlay_l042_s065",
    )
    ...
    print(f"[entry-overlay-compare] wrote {out}")
    print(f"[entry-overlay-compare] wrote {events_out}")
