...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare baseline exits vs baseline + intrabar ATR trailing exits."
    )
    parser.add_argument(
        "--meta-matrix",
    ...
    parser.add_argument("--trail-activate-atr", type=float, default=0.75, help="ATR move required to arm the intrabar trail.")
    parser.add_argument("--trail-atr", type=float, default=0.8, help="Base trailing distance in ATR once active.")
    parser.add_argument("--trail-tp-atr", type=float, default=1.0, help="ATR move that marks TP seen for trail tightening.")
    parser.add_argument("--trail-atr-after-tp", type=float, default=0.5, help="Tightened trailing distance after TP is seen.")
    parser.add_argument("--use-tp-to-tighten-trail", action=argparse.BooleanOptionalAction, default=True, help="Tighten trail after TP is seen.")
    parser.add_argument(
        "--trace-out",
        default="Data/inference/spy/10min/meta/meta_trace_baseline_atr_trail_compare.csv",
    ...
    parser.add_argument(
        "--variant-events-out",
        default="Data/inference/spy/10min/meta/meta_events_baseline_exit_atr_trail.csv",
        help="Variant event CSV.",
    )
    parser.add_argument(
        "--summary-out",
        default="Data/inference/spy/10min/meta/baseline_atr_trail_compare_summary.csv",
        help="Summary metrics CSV.",
    )
    parser.add_argument(
        "--equity-out",
        default="Data/inference/spy/10min/plots/baseline_atr_trail_compare_equity.png",
        help="Equity comparison PNG.",
    )
    parser.add_argument(
        "--out-dir",
        default="Data/inference/spy/10min/plots/baseline_atr_trail_compare_sessions",
...
def _run_regime(
    *,
    ...
    opposite_dominance_delta: float,
    trail_activate_atr: float | None,
    trail_atr: float | None,
    trail_tp_atr: float | None,
    trail_atr_after_tp: float | None,
    ...
    long_favorable_anchor = float("nan")
    short_favorable_anchor = float("nan")
    long_tp_seen = False
    short_tp_seen = False
    ...
    for idx, (_, row) in enumerate(meta_df.iterrows()):
        ...
        for _, bar in interval.iterrows():
            ...
            if (
                ...
                long_favorable_anchor = fill_price
                long_tp_seen = False
            ...
            if (
                ...
                short_favorable_anchor = fill_price
                short_tp_seen = False
                events.append({"timestamp": bar_ts, "symbol": symbol, "event": "enter_short", "price": fill_price, "reason": "next_10m_open"})

            if trail_activate_atr is not None and trail_atr is not None and float(trail_atr) > 0.0:
                if long_active and np.isfinite(long_entry_price) and np.isfinite(long_entry_atr) and long_entry_atr > 0.0:
                    if np.isfinite(bar_high):
                        long_favorable_anchor = max(float(long_favorable_anchor), float(bar_high)) if np.isfinite(long_favorable_anchor) else float(bar_high)
                    if (
                        not long_tp_seen
                        and trail_tp_atr is not None
                        and float(trail_tp_atr) > 0.0
                        and np.isfinite(bar_high)
                        and bar_high >= long_entry_price + float(trail_tp_atr) * long_entry_atr
                    ):
                        long_tp_seen = True
                    long_mfe_atr = (
                        (float(long_favorable_anchor) - long_entry_price) / long_entry_atr
                        if np.isfinite(long_favorable_anchor)
                        else float("nan")
                    )
                    long_trail_active = bool(np.isfinite(long_mfe_atr) and long_mfe_atr >= float(trail_activate_atr))
                    trail_dist = float(trail_atr)
                    if use_tp_to_tighten_trail and long_tp_seen and trail_atr_after_tp is not None and float(trail_atr_after_tp) > 0.0:
                        long_trail_active = True
                        trail_dist = min(trail_dist, float(trail_atr_after_tp))
                    if long_trail_active and np.isfinite(long_favorable_anchor):
                        trail_level = float(long_favorable_anchor) - trail_dist * long_entry_atr
                        if np.isfinite(bar_low) and bar_low <= trail_level:
                            fill_price = min(trail_level, bar_open) if np.isfinite(bar_open) else trail_level
                            long_active = False
                            long_intent_active = False
                            long_signal_row = None
                            long_soft_confirm = 0
                            ...
                            long_favorable_anchor = float("nan")
                            long_tp_seen = False
                            events.append({"timestamp": bar_ts, "symbol": symbol, "event": "exit_long", "price": fill_price, "reason": "atr_trail"})
                if short_active and np.isfinite(short_entry_price) and np.isfinite(short_entry_atr) and short_entry_atr > 0.0:
                    if np.isfinite(bar_low):
                        short_favorable_anchor = min(float(short_favorable_anchor), float(bar_low)) if np.isfinite(short_favorable_anchor) else float(bar_low)
                    if (
                        not short_tp_seen
                        and trail_tp_atr is not None
                        and float(trail_tp_atr) > 0.0
                        and np.isfinite(bar_low)
                        and bar_low <= short_entry_price - float(trail_tp_atr) * short_entry_atr
                    ):
                        short_tp_seen = True
                    short_mfe_atr = (
                        (short_entry_price - float(short_favorable_anchor)) / short_entry_atr
                        if np.isfinite(short_favorable_anchor)
                        else float("nan")
                    )
                    short_trail_active = bool(np.isfinite(short_mfe_atr) and short_mfe_atr >= float(trail_activate_atr))
                    trail_dist = float(trail_atr)
                    if use_tp_to_tighten_trail and short_tp_seen and trail_atr_after_tp is not None and float(trail_atr_after_tp) > 0.0:
                        short_trail_active = True
                        trail_dist = min(trail_dist, float(trail_atr_after_tp))
                    if short_trail_active and np.isfinite(short_favorable_anchor):
                        trail_level = float(short_favorable_anchor) + trail_dist * short_entry_atr
                        if np.isfinite(bar_high) and bar_high >= trail_level:
                            fill_price = max(trail_level, bar_open) if np.isfinite(bar_open) else trail_level
                            short_active = False
                            short_intent_active = False
                            short_signal_row = None
                            short_soft_confirm = 0
                            ...
                            short_favorable_anchor = float("nan")
                            short_tp_seen = False
                            events.append({"timestamp": bar_ts, "symbol": symbol, "event": "exit_short", "price": fill_price, "reason": "atr_trail"})
...
def _event_metrics(events: pd.DataFrame) -> dict[str, float]:
    ...
    trail_count = int(events["reason"].astype(str).eq("atr_trail").sum()) if "reason" in events.columns else 0
    return {
        ...
        "atr_trail_exit_count": float(trail_count),
    }
...
def _save_equity_plot(
    *,
    ...
    ax.plot(
        xv,
        variant_eq["net_1x_style"],
        color="#1565C0",
        linewidth=2.0,
        label=f"baseline_plus_trail_a{trail_activate_atr:.2f}_t{trail_atr:.2f}_tp{trail_tp_atr:.2f}_tt{trail_atr_after_tp:.2f}_net_1x",
    )
    ax.plot(xb, baseline_eq["variant_or_baseline"], color="#C62828", linewidth=1.4, label="baseline_hybrid_full_gross")
    ax.plot(
        xv,
        variant_eq["variant_or_baseline"],
        color="#8E24AA",
        linewidth=1.4,
        label=f"baseline_plus_trail_full_gross",
    )
    ax.set_title(f"{symbol} | next 10m open entries | baseline vs ATR trail exit")
    ...
    *,
    ...
    for chunk_idx in range(chunks):
        ...
        for idx, session_date in enumerate(dates):
            ...
            ax_price.set_title(
                f"{symbol} | {session_date.date()} | baseline vs ATR trail "
                f"(activate {trail_activate_atr:.2f}, trail {trail_atr:.2f}, tp {trail_tp_atr:.2f}, tighten {trail_atr_after_tp:.2f})"
            )
        ...
        fig.tight_layout()
        out_path = out_dir / f"{symbol.lower()}_baseline_atr_trail_part{chunk_idx + 1}.png"
...
def main() -> None:
    ...
    _, variant_events = _run_regime(
        ...
        trail_tp_atr=float(args.trail_tp_atr),
    ...
    summary = pd.DataFrame(
        [
            ...
            {
                "regime": (
                    f"baseline_plus_atr_trail_a{float(args.trail_activate_atr):.2f}"
                    f"_t{float(args.trail_atr):.2f}_tp{float(args.trail_tp_atr):.2f}"
                    f"_tt{float(args.trail_atr_after_tp):.2f}"
                ),
                **_event_metrics(variant_events),
            },
        ]
    )
