...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot mixed policy/meta replay trace with 1m candles and 10m meta probabilities by session."
    )
    parser.add_argument(
        "--trace",
    ...
    parser.add_argument(
        ...
        help="Display timezone.",
    )
    parser.add_argument(
        "--sessions-per-fig",
        type=int,
        default=4,
        help="Number of sessions per output figure.",
    )
    parser.add_argument(
        "--out-dir",
        default="Data/inference/spy/10min/plots/policy_sessions",
        help="Directory for session plot PNGs.",
    )
...
def _load_trace(path: Path, *, symbol: str, start: str | None, end: str | None, tz: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    policy = df[df["trace_kind"].astype(str) == "policy_1m"].copy()
    meta = df[df["trace_kind"].astype(str) == "meta_10m"].copy()
    return policy, meta


def _derive_policy_markers(policy_df: pd.DataFrame) -> pd.DataFrame:
    out = policy_df.sort_values("ts_local").copy()
...
def _plot_sessions(policy: pd.DataFrame, meta: pd.DataFrame, *, out_dir: Path, sessions_per_fig: int, tz: str, symbol: str) -> list[Path]:
    ...
    if not session_dates:
        raise ValueError("No policy_1m session rows found to plot.")
    ...
    for chunk_idx in range(chunks):
        ...
        for idx, session_date in enumerate(dates):
            ...
            for col, yvals, color, marker, label in (
                ...
                if mask.any():
                    ax_price.scatter(x[mask], yvals[mask], color=color, marker=marker, s=34, label=label, zorder=2.0)

            ax_price.set_title(f"{symbol} | {session_date.date()} | 1m policy execution")
        ...
        fig.tight_layout()
        out_path = out_dir / f"{symbol.lower()}_policy_sessions_part{chunk_idx + 1}.png"
